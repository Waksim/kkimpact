...
from keyboards.ru import kb_draft_queue, kb_draft_tail_accept_decline, kb_draft_tail_who_win, kb_start_blep_drafts
...
def create_queue_answer(users_data, user_id):
    queue_answer = f"😼 Блеп-Драфты\n{html.bold('Список игроков в подборе:')}\n\n"
...
async def update_queue():
    ...
    cursor.execute(
        f"SELECT user_id, message_id, username, firstname, requested FROM main.blep_drafts_queue WHERE in_the_queue = 1"
    )
        ...
        for user in users_data:
            ...
            try:
                await bot.edit_message_text(
                    ...
                    reply_markup=kb_draft_queue(users_data, user_id, 1),
...
@blep_drafts.message(F.text == "😼 Блеп-Драфты")
async def show_draft_menu(message: types.Message, state: FSMContext):
    ...
    cursor.execute(
        "SELECT EXISTS(SELECT * FROM main.blep_drafts_queue where user_id = ?)", (message.from_user.id,)
    )
    is_exists = cursor.fetchall()[0][0]
    if is_exists == 0:
        cursor.execute(
            f"INSERT INTO main.blep_drafts_queue (user_id, username, firstname, in_the_queue) "
            f"VALUES (?, ?, ?, ?)",
        ...
        cursor.execute(
            f"UPDATE main.blep_drafts_queue SET in_the_queue = 1 "
            f"WHERE user_id = ?",
            (message.from_user.id,)
        )
        sqlite_connection.commit()

    cursor.execute(f"SELECT user_id, message_id, username, firstname, requested FROM main.blep_drafts_queue "
    ...
    message_obj = await message.answer(
        queue_answer,
        parse_mode=ParseMode.HTML,
        reply_markup=kb_draft_queue(users_data, message.from_user.id, 1),
        disable_web_page_preview=True
    )
    cursor.execute(
        "UPDATE main.blep_drafts_queue SET datatime = ?, message_id = ? WHERE user_id = ?",
...
@blep_drafts.callback_query(F.data == "choose_opponent_alert")
...
@blep_drafts.callback_query(F.data == "b_update_queue_list")
async def b_update_queue_list(callback: types.CallbackQuery):
    sqlite_connection = sqlite3.connect('./users_info.sqlite')
    cursor = sqlite_connection.cursor()
...
async def go_to_main_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    cursor = sqlite_connection.cursor()
    cursor.execute(
        f"DELETE FROM main.blep_drafts_queue WHERE user_id = {user_id}"
    )
    sqlite_connection.commit()
...
@blep_drafts.callback_query(F.data.startswith("b_draft_tail_request="))
async def b_draft_tail_request(callback: types.CallbackQuery):
    ...
    if requested is None:
        ...
        cursor.execute(
            f"UPDATE main.blep_drafts_queue SET requested = '{requested}' WHERE user_id = {user_id_1}"
        )
        sqlite_connection.commit()

        await bot.send_message(
            chat_id=user_id_2,
            text=f"Пользователь {firstname_1}, @{username_1}: приглашает вас сыграть в драфты Блепа.",
            reply_markup=kb_draft_tail_accept_decline(user_id_1, username_1, 1)
        )
        await update_queue()
        return
...
@blep_drafts.callback_query(F.data.startswith("b_draft_tail_declined="))
async def b_draft_tail_declined(callback: types.CallbackQuery):
...
@blep_drafts.callback_query(F.data.startswith("b_draft_tail_accepted="))
async def b_draft_tail_accepted(callback: types.CallbackQuery, state: FSMContext):
    await bot.delete_message(chat_id=callback.from_user.id, message_id=callback.message.message_id)
    ...
    for current_player in [first_player, second_player]:
        cursor.execute(
            f"INSERT INTO main.blep_drafts_history (user_id, datatime, opponent, deck_code)"
            f"VALUES (?, ?, ?, ?)",
            (current_player['user_id'], int(datetime.now().timestamp()), current_player['opponent'], '0')
        )
        sqlite_connection.commit()
        ...
        message_obj = await bot.send_message(chat_id=current_player['user_id'],
                               ...
                               reply_markup=kb_start_blep_drafts(current_player['user_id'], current_player['opponent']))
                               #               )

        cursor.execute(
            f"UPDATE main.blep_drafts_queue SET requested = NULL, in_the_queue = 0, message_id = {message_obj.message_id} "
            f"WHERE user_id = {current_player['user_id']}"
        )
        sqlite_connection.commit()

        cursor.execute(
            f"UPDATE main.blep_drafts_history SET message_id = {message_obj.message_id} "
            f"WHERE user_id = {current_player['user_id']}"
        )
        sqlite_connection.commit()
    ...
    await state.set_state('Blep_Drafts')
...
async def add_new_card_to_drafts_data(card_code, bans_or_picks, bans_or_picks_arr, user_id, draft_data):
    ...
    if card_name not in draft_data_in_row:
        ...
        cursor.execute(
            f"UPDATE main.blep_drafts_history SET {bans_or_picks} = '{json_str}' WHERE user_id = {user_id}"
        )
        sqlite_connection.commit()
        return draft_data
    else:
...
async def stage_update(stage, user_id):
    ...
    cursor.execute(
        f"UPDATE main.blep_drafts_history SET stage = '{stage}' WHERE user_id = {user_id}"
    )
    sqlite_connection.commit()
...
async def update_drafts(user_id_1, user_id_2, fp_status, scp_status, stage, cardcode):
    ...
    for user_id in (user_id_1, user_id_2):
        cursor.execute(
            f"SELECT message_id, picks, bans FROM main.blep_drafts_history WHERE user_id = {user_id} ORDER BY id DESC"
        )
        r = cursor.fetchall()
        if len(r) == 0:
            draft_data.append([])
    ...
    for status in [fp_status, scp_status]:
        ...
        cursor.execute(
            f"UPDATE main.blep_drafts_history SET draft_data = '{json.dumps(draft_data)}' WHERE user_id = {users_ids[c]}"
        )
        sqlite_connection.commit()
...
# @blep_drafts.message(F.sticker, StateFilter('DraftTail'))
@blep_drafts.message(F.sticker)
async def show_draft_menu(message: types.Message, state: FSMContext):
    ...
    cursor.execute(
        ...
        f" FROM main.blep_drafts_history WHERE user_id = '{message.from_user.id}'"
        f"ORDER BY id DESC"
    )
    r = cursor.fetchall()
    if len(r) != 0:
                ...
                if await add_new_card_to_drafts_data(card_code, 'picks', picks, user_id, draft_data):
                    ...
                    for status in [fp_status, scp_status]:
                        ...
                        message_obj = await bot.edit_message_media(
                            chat_id=users_ids[c],
                            message_id=message_ids[c],
                            reply_markup=kb_draft_tail_who_win(opponent_ids[c], 1),
                            media=media_photo
                        )
                        c += 1
                    ...
                    sqlite_connection.commit()
                    cursor.execute(
                        f"DELETE FROM main.blep_drafts_history WHERE user_id = {users_ids[0]} OR user_id = {users_ids[1]}"
                    )
                    sqlite_connection.commit()
...
@blep_drafts.callback_query(F.data == "draft_tail_rules")
...
@blep_drafts.callback_query(F.data.startswith("draft_with_bot"))
async def draft_with_bot(callback: types.CallbackQuery, state: FSMContext):
    ...
    message_id = message_obj.message_id
    cursor.execute(
        f"INSERT INTO main.blep_drafts_history (user_id, message_id, first_pick, datatime, opponent, stage, draft_data)"
        f"VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    cursor.execute(
        f"UPDATE main.blep_drafts_queue SET requested = NULL, in_the_queue = 0 "
...
@blep_drafts.message(F.sticker, StateFilter('DraftTailBOT'))
...
#
#
# @blep_drafts.message(DraftTail.your_pick, F.sticker)
...
# @blep_drafts.message(DraftTail.your_ban, F.sticker)
# async def get_ban(message: types.Message, state: FSMContext):
#
#
# @blep_drafts.message(DraftTail.enemy_pick, F.sticker)
# async def wait_pick(message: types.Message, state: FSMContext):
#
#
# @blep_drafts.message(DraftTail.enemy_ban, F.sticker)
